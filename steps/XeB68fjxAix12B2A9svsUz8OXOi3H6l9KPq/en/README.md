...
* To enable `2-Factor Authentication`, we have two methods in GitLab: **One-time password authenticator** and **WebAuthn device**.
...
1. Before proceeding further, test the new SSH key to ensure it works as expected.
...
1. If you're using personal access tokens for authentication, consider rotating them as well.
...
* Here's an example of how to automate the process of rotating SSH tokens and personal keys in GitLab using JavaScript:
...
* In this JavaScript example:
  ...
  * You can always read the GitLab API references to understand the working of API endpoints.

## Tracking Activities via Audit Logs on GitLab

* There are two types of auditing options in GitLab. One is **Authentication Log** and other is **Audit Events**. In below sections, we will understand to access and filter both of these logs.
...
* APIs can also be used to fetch the audit records
...
* To take backups of repositories in GitLab, utilize GitLab's built-in backup functionality. Here are the steps to take a backup of your GitLab projects:
...
* There are 3 types of repos in GitLab:
  ...
  * **Public:** unauthenticated users, including users with the Guest role, can clone the project. View the public access directory (/public). Public groups can have public, internal, or private subgroups and projects.
...
## Using Pre-commit hook to scans repositories on GitLab
...
1. Create or update your `.gitlab-ci.yml` file to include a job that runs the same checks as the pre-commit hook. This ensures that code pushed to GitLab is also scanned during the CI process. Here's an example configuration:
...
1. Create or update your `.gitlab-ci.yml` file (You can use the pipeline editor) to include jobs that run the necessary checks. Define stages and jobs for each type of check you want to perform. The below code snippet is the example for this:
...
1. Configure GitLab CI to run jobs only on pull requests(In GitLab it is called as merge request). Modify the above code and add the only property below every stage:
...
1. Create or update your `.gitlab-ci.yml` file to include a job that updates dependencies. The below code snippet is the example using npm for node projects
    ...
    * This configuration creates two jobs named `update_dependencies` and `push_artifacts`  in two stage process. It uses the `node:14` Docker image, installs npm package `ncu` in turn it updates the `package.json` dependencies in the script. Just ensure that you also add `package.json` file the root of your repository.
