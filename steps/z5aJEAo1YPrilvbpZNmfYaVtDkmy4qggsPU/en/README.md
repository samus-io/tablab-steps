...
* Check if the URL is one among the below two options:
  * **URLs Relative to the web root**: redirects should use URLs relative to the web root whenever possible. The redirection function should validate that the received URL starts with a slash character (`/`) to ensure it's relative to the web root. Before issuing the redirect, prepend `http://yourdomainname.org` to the URL.
  * **Absolute URLs with domain validation**: if absolute URLs are necessary, ensure they start with required domain name like `http://yourdomainname.org/`. The redirection function should verify that the user-supplied URL begins with genuine domain before issuing the redirect. If multiple domains are allowed, create a whitelist to restrict redirection to trusted domains.
...
* An example of this behavior can be experienced by adding `https://www.google.com/url?q=https://example.com` to your browser's address bar.
